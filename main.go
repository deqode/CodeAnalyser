...
func main() {
	...
	for _, language := range languages {
		...
		if detectors, ok := frameworks.Languages[language.Name]; ok {
			detectors.Prerequisites(root) // Think some other names
			...
			for _, dbChecker := range detectors.DBCheckers { // DBCheckers Think more generic names
				_, db, port := dbChecker.Detector(root)
				log.Println(dbChecker.Name, db, port)
			}
			for _, orm := range detectors.Orms { // Libraries
...
// Todo: decide folder structure first
// Todo: declare interfaces
